//
//  UIImageView+ZgyCornerRadiusExtension.m
//  BSBDJ-ZGY
//
...
@end
@implementation UIImageView (ZgyCornerRadiusExtension)
...
- (instancetype)initWithRoundingRectImageView {
    self = [super init];
    if (self) {
        [self zg_cornerRadiusRoundingRect];
    }
    return self;
}
...
- (instancetype)initWithCornerRadiusAdvance:(CGFloat)cornerRadius rectCornerType:(UIRectCorner)rectCornerType {
    self = [super init];
    if (self) {
        [self zg_cornerRadiusAdvance:cornerRadius rectCornerType:rectCornerType];
    }
    return self;
}
 ...
 */
- (void)zg_cornerRadiusWithImage:(UIImage *)image cornerRadius:(CGFloat)cornerRadius rectCornerType:(UIRectCorner)rectCornerType {
 ...
 */
- (void)zg_cornerRadiusWithImage:(UIImage *)image cornerRadius:(CGFloat)cornerRadius rectCornerType:(UIRectCorner)rectCornerType backgroundColor:(UIColor *)backgroundColor {
    CGSize size = self.bounds.size;
    CGFloat scale = [UIScreen mainScreen].scale;
    CGSize cornerRadii = CGSizeMake(cornerRadius, cornerRadius);
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"image"]) {
        ...
        if (self.isRounding) {
            [self zg_cornerRadiusWithImage:newImage cornerRadius:self.frame.size.width/2 rectCornerType:UIRectCornerAllCorners];
        } else if (0 != self.radius && 0 != self.roundingCorners && nil != self.image) {
            [self zg_cornerRadiusWithImage:newImage cornerRadius:self.radius rectCornerType:self.roundingCorners];
        }
    }
}
