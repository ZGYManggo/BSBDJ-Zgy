...
const CGFloat ZgRefreshHeaderHeight = 35.0;
const CGFloat ZgRefreshPointRadius = 5.0;

const CGFloat ZgRefreshPullLen     = 55.0 ;
const CGFloat ZgRefreshTranslatLen = 5.0;
...
@end
@implementation ZgRefreshHeader

- (instancetype)init {
    if (self = [super initWithFrame:CGRectMake(0, 0, ZgRefreshHeaderHeight, ZgRefreshHeaderHeight)]) {
...
- (void)initLayers {
    CGFloat centerLine = ZgRefreshHeaderHeight / 2;
    CGFloat radius = ZgRefreshPointRadius;
    //
    ...
    CGPoint bottomPoint = CGPointMake(centerLine, ZgRefreshHeaderHeight - radius);
    self.BottomPointLayer = [self layerWithPoint:bottomPoint color:bottomPointColor];
    [self.layer addSublayer:self.BottomPointLayer];
    
    CGPoint rightPoint = CGPointMake(ZgRefreshHeaderHeight - radius, centerLine);
    ...
    self.lineLayer.lineWidth = ZgRefreshPointRadius * 2;
...
- (CAShapeLayer *)layerWithPoint:(CGPoint)center color:(CGColorRef)color {
    ...
    layer.frame = CGRectMake(center.x - ZgRefreshPointRadius, center.y - ZgRefreshPointRadius, ZgRefreshPointRadius * 2, ZgRefreshPointRadius * 2);
...
- (CGPathRef)pointPath {
    return [UIBezierPath bezierPathWithArcCenter:CGPointMake(ZgRefreshPointRadius, ZgRefreshPointRadius) radius:ZgRefreshPointRadius startAngle:0 endAngle:M_PI * 2 clockwise:YES].CGPath;
}
...
- (void)willMoveToSuperview:(UIView *)newSuperview {
    ...
    if (newSuperview) {
        ...
        self.center = CGPointMake(self.scrollView.zg_centerX, self.zg_centerY);
...
- (void)setProgress:(CGFloat)progress {
    progress -= 99.f;
    _progress = progress ;
//    NSLog(@"%0.2f",_progress);
    //如果不是正在刷新，则渐变动画
    if (!self.animating) {
        if (progress >= ZgRefreshPullLen) {
            self.zg_y = - (ZgRefreshPullLen - (ZgRefreshPullLen - ZgRefreshHeaderHeight) / 2);
        }else {
            if (progress <= self.zg_height) {
                self.zg_y = - progress;
            }else {
                self.zg_y = - (self.zg_height + (progress - self.zg_height) / 2);
            }
        }
        [self setLineLayerStrokeWithProgress:progress];
    }
    //如果到达临界点，则执行刷新动画
    if (progress >= ZgRefreshPullLen && !self.animating && !self.scrollView.dragging) {
        [self startAni];
        if (self.handle) {
            self.handle();
        }
...
- (void)setLineLayerStrokeWithProgress:(CGFloat)progress {
    ...
    else if (progress >= 0 && progress < (ZgRefreshPullLen - 40)) {
        self.TopPointLayer.opacity = progress / 20;
        [self adjustPointStateWithIndex:0];
    }
    else if (progress >= (ZgRefreshPullLen - 40) && progress < ZgRefreshPullLen) {
        self.TopPointLayer.opacity = 1.0;
        //大阶段 0 ~ 3
        NSInteger stage = (progress - (ZgRefreshPullLen - 40)) / 10;
        //大阶段的前半段
        CGFloat ZgbProgress = (progress - (ZgRefreshPullLen - 40)) - (stage * 10);
//        NSLog(@"%ld,%0.2f",(long)stage,ZgbProgress);
        if (ZgbProgress >= 0 && ZgbProgress <= 5) {
            [self adjustPointStateWithIndex:stage * 2];
            startProgress = stage / 4.0;
            endProgress = stage / 4.0 + ZgbProgress / 40.0 * 2;
        }
        //大阶段的后半段
        if (ZgbProgress > 5 && ZgbProgress < 10) {
            [self adjustPointStateWithIndex:stage * 2 + 1];
            startProgress = stage / 4.0 + (ZgbProgress - 5) / 40.0 * 2;
...
//    NSLog(@"start = %0.2f,end = %0.2f",startProgress,endProgress);
...
- (void)startAni {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        inset.top += ZgRefreshPullLen;
        self.scrollView.contentInset = inset;
    }];
    [self addTranslationAniToLayer:self.TopPointLayer xValue:0 yValue:ZgRefreshTranslatLen];
    [self addTranslationAniToLayer:self.LeftPointLayer xValue:ZgRefreshTranslatLen yValue:0];
    [self addTranslationAniToLayer:self.BottomPointLayer xValue:0 yValue:-ZgRefreshTranslatLen];
    [self addTranslationAniToLayer:self.rightPointLayer xValue:-ZgRefreshTranslatLen yValue:0];
